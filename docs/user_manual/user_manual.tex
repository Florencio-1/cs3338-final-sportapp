...
Project tasks and sprints will be tracked in Jira:

\begin{itemize}
  \item Jira board: \texttt{TBD -- Jira project not created yet. The link will
        be added in a later snapshot.}
\end{itemize}
...
\section{Current Status (Checkpoint 2)}

As of Snapshot 3 (Checkpoint 2), the focus is on finishing the
\textbf{Favorites} experience. The All Games view retains sport filters
(All, NBA, NFL, MLB, Soccer) and now includes a favorite toggle. A simple
\textbf{My Favorites} view shows the games the user marked, stored on the
frontend for now while backend persistence is designed. Admin features remain
planned for later snapshots. TestRail coverage now includes favorite/unfavorite
and viewing the favorites list (results captured in the Snapshot 3 report).
...
\begin{itemize}
  \item Jira board: \url{https://calstatela-team4.atlassian.net/jira/software/projects/ST/boards/68?sprintStarted=true}
\end{itemize}
