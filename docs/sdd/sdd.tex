...
\begin{itemize}
  ...
  \item \textbf{Backend API (server)} – A Node.js/Express application that
        ...
        \texttt{/api/games}, \texttt{/api/teams}, \texttt{/api/auth} and
        \texttt{/api/favorites}. It checks requests, talks to the database, and
        sends back responses to the frontend.
  \item \textbf{Database} – A PostgreSQL database that stores persistent data:
        users, teams, games, and which teams each fan has favorited.
\end{itemize}
...
\begin{itemize}
  ...
  \item The \textbf{Register} page lets a new user create an account by
        entering an email and password. After registration they can log in and
